...
let countryPng;
const GeoapiKey = '3adff1b36f8a4a9284de06a26584a84f'; 
...
let response3 = await fetch(GeoapiUrl + `q=${data.coord.lat}+${data.coord.lon}&key=${GeoapiKey}`)
let data2 = await response3.json()
console.log(data);
console.log(data1);
console.log(data2);
console.log(data2.results[0].components.state);

document.querySelector(".State").innerHTML = `State : ${data2.results[0].components.state} `
...
document.querySelector( ".weather-icon").src = `images/${data.weather[0].main}.png`|| `${data.weather[0].description}`;
...
document.querySelector(".time").innerHTML = showTimefn(data.dt,data.timezone);  
    ...
    setTimeout(() => {
      searchValue.setAttribute("placeholder", "");
    }, 5000);
